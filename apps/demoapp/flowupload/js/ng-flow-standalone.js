...
(function(window, document, undefined) {'use strict';
  ...
  Flow.prototype = {
    ...
    generateUniqueIdentifier: function (file) {
      ...
      return file.size + '-' + n + '-' + relativePath.replace(/[^0-9a-zA-Z_-]/img, '');
    ...
    upload: function () {
      ...
      for (var num = 1; num <= this.opts.simultaneousUploads - ret; num++) {
        ...
        started = this.uploadNextChunk(true);
      }
    ...
    addFiles: function (fileList, event) {
      ...
      each(fileList, function (file) {
        ...
        if ((!ie10plus || ie10plus && file.size > 0) && !(file.size % 4096 === 0 && (file.name === '.' || file.fileName === '.'))) 
		{
			var $this = this;
			var self = this;
			this.checkFileExist('/nc/index.php/apps/files/ajax/checkfile.php', file, function(data){
				console.log(data);
				var resp = data;
				if(resp==1)
				{
					self.filesClient.getFileInfo(file.name).then(function(status, fileInfo) {
						var original = fileInfo;
						var replacement = file;
						var dir = self.prepareDirUrl();
						original.directory = dir; 
						fileExistCount++;
						OC.dialogs.popup_fileexists(file, original, replacement, self, fileExistCount);
					});
				}
				else
				{
					var uniqueIdentifier = self.generateUniqueIdentifier(file);
					if (self.opts.allowDuplicateUploads || !self.getFromUniqueIdentifier(uniqueIdentifier)) 
					{
						var f = new FlowFile(self, file, uniqueIdentifier);
						if (self.fire('fileAdded', f, event)) {
							files.push(f);
						}
					}
				}
	...
	addSingleFile: function (file, event, self) {
		var files = [];
		var uniqueIdentifier = self.generateUniqueIdentifier(file);
		if (self.opts.allowDuplicateUploads || !self.getFromUniqueIdentifier(uniqueIdentifier))
		{
	...
	prepareDirUrl: function(){
		var $url = document.getElementById("dir").value;
		if($url!='')
		{
			st_dirurl= $url;
		}
	...
	checkFileExist: function(url, data, success) {
		
		var relativePath = data.relativePath || data.webkitRelativePath || data.fileName || data.name;
		var $url = this.prepareDirUrl();
		var params = 'relativePath='+relativePath+'&dir='+$url;
  ...
  function FlowFile(flowObj, file, uniqueIdentifier) {
     ...
     * @type {string}
     */
    this.name = file.newname || file.fileName || file.name;
     ...
     * @type {string}
     */
    this.relativePath = file.newpath || file.relativePath || file.webkitRelativePath || this.name;
