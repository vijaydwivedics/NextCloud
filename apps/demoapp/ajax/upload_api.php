...
function nc_getSharePath($path='',$header=null)
{
	...
	$name = trim($result['ocs']['data']['file_target'],'/');
	return array('path'=>$path,'name'=>$name,'share_path'=>$share_url);
...
function nc_getdir($postData)
{
	...
	if(!empty($postData) && isset($postData['path']) &&$postData['path']!='')
	{
		...
		//$dir_path_n = rawurlencode(trim($dir_path,'/'));
		$dir_path_n = str_replace(' ', '%20', $dir_path);
		//exit;
		$url = "/nc/remote.php/webdav/".$dir_path_n;
		...
		$result = json_encode(array('status' => 'success','error' => 0,'message' => 'Success', 'data'=>$xml));
		$responce = json_decode($result,true);
		
		$structure = array();
		foreach($responce['data']['response'] as $data)
		{
			$path = pathinfo($data['href']);
			$sharepath = urldecode(trim(end(explode('/nc/remote.php/webdav/',$data['href'])),'/'));
			
			if($path['basename'] !='webdav' &&  '/'.$path['basename'] != $dir_path && $dir_path != '/'.$sharepath)
			{
				$type = 'dir';
				if (isset($path['extension']) && $path['extension']!='')
					$type = 'file';
				
				if (array_key_exists(0,$data['propstat'])){
					$prop = $data['propstat'][0]['prop'];
				}
				else{
					$prop = $data['propstat']['prop'];
				}
				
				$lastmodified = $prop['getlastmodified'];
				$size = 0;
				if(!empty($prop['quota-used-bytes'])){
					$size = (int)$prop['quota-used-bytes'];
				}
				else{
					if(empty($prop['quota-used-bytes']) && isset($prop['getcontentlength'])){
				...
				$getoutput = nc_getSharePath($sharepath,$header);
				$ar = array('href'=>'/'.$sharepath,'name'=>urldecode($path['basename']),'type'=>$type,'size'=>$size,'shareurl'=>$getoutput,'lastmodified'=>$lastmodified);
				array_push($structure,$ar);
			}
		}
		usort($structure, 'compareByName');
		usort($structure, 'compareByType');
		$response = array('status' => 'success','error' => 0,'message' => 'Success', 'data'=>$structure);
	}
	return $response;
	exit;
}
...
function nc_checkuser($postData)
{
	...
	$log = json_encode(array('result'=>$response,'user'=>$postData['userid'],'password'=>$postData['password']));
...
function ocs_request_nextcloud($header,$data) {
	...
	$curl = curl_init(BASE_URL.'/nc/'.$nc_url);
	curl_setopt($curl, CURLOPT_HTTPHEADER, $header);
	
	//if (key_exists('put', $data)) {}
	curl_setopt($curl, CURLOPT_CUSTOMREQUEST, "PUT");
	curl_setopt($curl, CURLOPT_POSTFIELDS, $nc_data);
